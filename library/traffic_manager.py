...
def get(password, name):
    response = requests.get(
        "http://localhost:9070/api/tm/3.10/config/active/traffic_managers/{}".format(name),
        auth=("admin", password)
    )
...
def sort_table(section, field, table):
    table_keys = {"basic": {"appliance_card": ["name", "interfaces"], "appliance_sysctl": ["sysctl", "value"], "trafficip": ["name", "networks"]}, "appliance": {"hosts": ["name", "ip_address"], "if": ["name"], "ip": ["name", "addr", "mask"], "routes": ["name", "gw", "if", "mask"]}}
