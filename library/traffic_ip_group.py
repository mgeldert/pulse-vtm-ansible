...
def get(password, name):
    response = requests.get(
        "http://localhost:9070/api/tm/3.10/config/active/traffic_ip_groups/{}".format(name),
        auth=("admin", password)
    )
...
def sort_table(section, field, table):
    table_keys = {"basic": {"ip_mapping": ["ip", "traffic_manager"]}}
