...
def get(password, name):
    response = requests.get(
        "http://localhost:9070/api/tm/3.10/config/active/glb_services/{}".format(name),
        auth=("admin", password)
    )
...
def sort_table(section, field, table):
    table_keys = {"basic": {"dnssec_keys": ["domain", "ssl_key"], "location_settings": ["location", "ips"]}}
