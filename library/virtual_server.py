...
def get(password, name):
    response = requests.get(
        "http://localhost:9070/api/tm/3.10/config/active/virtual_servers/{}".format(name),
        auth=("admin", password)
    )
...
def sort_table(section, field, table):
    table_keys = {"aptimizer": {"profile": ["name", "urls"]}, "ssl": {"ocsp_issuers": ["issuer"], "server_cert_host_mapping": ["host", "certificate"]}}
